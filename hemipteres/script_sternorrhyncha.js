// Script pour la page Sternorrhyncha
...
// Rendu des espèces de Sternorrhyncha
function renderSpecies() {
    const container = document.getElementById('species-container');
    
    if (!window.speciesData || !window.speciesData.sternorrhyncha) {
    ...
    const familyOrder = ['aphididae', 'psyllidae', 'coccidae', 'diaspididae', 'aleyrodidae'];
    
    familyOrder.forEach(familyKey => {
        if (window.speciesData.sternorrhyncha[familyKey] && window.speciesData.sternorrhyncha[familyKey].length > 0) {
            const familySection = createFamilySection(familyKey, familyNames[familyKey], window.speciesData.sternorrhyncha[familyKey]);
...
function findSpeciesById(id) {
    for (const family in window.speciesData.sternorrhyncha) {
        const species = window.speciesData.sternorrhyncha[family].find(s => s.id === id);
