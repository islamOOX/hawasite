// Script pour la page Hétéroptères
...
// Rendu des espèces d'Hétéroptères
function renderSpecies() {
    const container = document.getElementById('species-container');
    
    if (!window.speciesData || !window.speciesData.heteroptera) {
    ...
    const familyNames = {
        ...
        'scutelliridae': 'Scutelliridae',
        'tingidae': 'Tingidae'
    };
    
    // Ordre des familles pour l'affichage
    const familyOrder = ['pentatomidae', 'scutelliridae', 'tingidae'];
    
    familyOrder.forEach(familyKey => {
        if (window.speciesData.heteroptera[familyKey] && window.speciesData.heteroptera[familyKey].length > 0) {
            const familySection = createFamilySection(familyKey, familyNames[familyKey], window.speciesData.heteroptera[familyKey]);
...
function findSpeciesById(id) {
    for (const family in window.speciesData.heteroptera) {
        const species = window.speciesData.heteroptera[family].find(s => s.id === id);
