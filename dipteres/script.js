// Script principal pour la page Diptères
...
function initializeParticles() {
    ...
    const particleCount = 20; // Réduit de 50 à 20
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        particle.style.animationDelay = Math.random() * 8 + "s"; // Augmenté de 6 à 8
        particle.style.animationDuration = (Math.random() * 4 + 4) + "s"; // Augmenté de 3+3 à 4+4
...
function renderSpecies() {
    ...
    familyOrder.forEach(familyKey => {
        const speciesInFamily = Object.values(speciesData).filter(s => s.family === familyKey);
        if (speciesInFamily.length > 0) {
            const familySection = createFamilySection(familyKey, familyNames[familyKey], speciesInFamily);
...
function createSpeciesCard(species) {
    ...
    card.dataset.speciesId = species.name;

    // Image de l'espèce
    const image = document.createElement("img");
    image.className = "species-image";
    image.src = species.image || "https://via.placeholder.com/300x200/667eea/ffffff?text=Image+non+disponible";
    ...
    scientificName.innerHTML = `<em>${species.name}</em>`;

    const author = document.createElement("div");
    author.className = "species-author";
    author.textContent = species.scientificName;
...
function showTooltip(event, species, tooltip) {
    ...
    titleElement.innerHTML = `<em>${species.name}</em> ${species.scientificName}`;
    
    let content = `
        <div class="tooltip-content"><strong>Famille:</strong> ${species.family}</div>
        <div class="tooltip-content"><strong>Taille:</strong> ${species.size}</div>
        <div class="tooltip-content"><strong>Couleur:</strong> ${species.color}</div>
        <div class="tooltip-content"><strong>Habitat:</strong> ${species.habitat}</div>
        <div class="tooltip-content"><strong>Caractéristiques:</strong><ul>`;

    species.characteristics.forEach(char => {
        content += `<li>${char}</li>`;
    });
    content += `</ul></div>`;
